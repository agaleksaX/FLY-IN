...
class Parser:
    ...
    def _create_zone(self, token: Token) -> Zone:
        ...
        if len(token.values) != 3:
            raise ParserError(
                f"Line {token.line}: Invalid zone definition."
            )
    ...
    def _parse_zone(self, token: Token) -> None:
        ...
        if zone.name in self._zones:
            raise ParserError(
                f"Line {token.line}: Duplicate zone '{zone.name}'."
            )
    ...
    def _parse_start(self, token: Token) -> None:
        ...
        if self._start is not None:
            raise ParserError(
                f"Line {token.line}: Multiple start hubs defined."
            )
    ...
    def _parse_end(self, token: Token) -> None:
        ...
        if self._end is not None:
            raise ParserError(
                f"Line {token.line}: Multiple end hubs defined."
            )
    ...
    def _parse_connection(self, token: Token) -> None:
        ...
        if len(token.values) != 1:
            raise ParserError(
                f"Line {token.line}: Invalid connection definition."
            )
    ...
    def _parse_nb_drones(self, token: Token) -> None:
        ...
        if self._nb_drones != 0:
            raise ParserError(
                f"Line {token.line}: Duplicate nb_drones definition."
            )

        if len(token.values) != 1:
            raise ParserError(
                f"Line {token.line}: Invalid nb_drones definition."
            )
        ...
        if nb_drones <= 0:
            raise ParserError(
                f"Line {token.line}: Number of drones must be positive."
            )
    ...
    def _build_connections(self) -> None:
        ...
        for token in self._connections:
            ...
            if zone1_name not in self._zones:
                raise ParserError(
                    f"Line {token.line}: Unknown zone '{zone1_name}'."
                )

            if zone2_name not in self._zones:
                raise ParserError(
                    f"Line {token.line}: Unknown zone '{zone2_name}'."
                )
